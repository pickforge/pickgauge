...
# directory. Never uses sudo. Linux (AppImage with FUSE fallback) and macOS
# (.app) only.
set -eu
...
write_appimage_wrapper() {
  ...
  {
    printf '#!/bin/sh\n'
    printf '# PickGauge AppImage launcher generated by the PickGauge installer.\n'
    printf 'set -eu\n'
    printf "appimage_path='%s'\n" "$quoted_appimage_path"
    printf 'if [ ! -x "$appimage_path" ]; then\n'
    printf '  printf '"'"'PickGauge AppImage not found or not executable: %%s\\n'"'"' "$appimage_path" >&2\n'
    printf '  exit 127\n'
    printf 'fi\n'
    printf 'has_fuse2() {\n'
    ...
    printf 'fi\n'
    printf 'cache_root="${XDG_CACHE_HOME:-$HOME/.cache}/pickgauge/appimage-runtime"\n'
...
ensure_replaceable_command_path() {
  ...
  if [ -f "$command_path" ] &&
    grep -q 'PickGauge AppImage launcher generated by the PickGauge installer' "$command_path" 2>/dev/null; then
    return 0
  fi
  ...
  die "command path already exists and was not created by PickGauge: $command_path"
}
...
install_appimage() {
  ...
  write_desktop_launcher "$command_path" "$launcher_icon" || true
  refresh_desktop_caches
