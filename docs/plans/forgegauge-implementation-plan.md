...
Headless official refresh progress, 2026-06-04: split the Playwright sidecar into headed `launchLogin` for explicit user login and headless `refreshUsage` for normal official usage refresh checks. The desktop `Refresh usage`, `Refresh official`, and scheduled due-refresh paths now use the headless sidecar with the existing app-owned persistent profile, convert sanitized sidecar page state/visible fields through the web parser, update the normal provider cache/merge path, and report login-required states without flashing a visible browser. Scheduled headless web refreshes do not consume the manual web-refresh cooldown. `npm run test:official-fail-closed` validates blank Codex/Claude profiles return sanitized `logged_out` fail-closed states and forced dead-proxy Codex/Claude refreshes return sanitized `network_unavailable`, all with `headless: true` and `visibleBrowserRequired: false` on CachyOS KDE/Wayland. Authenticated official parsing, post-login session persistence, and saved-credential validation remain unchecked.

Official network fail-closed coverage progress, 2026-06-04: extended `npm run test:official-fail-closed` so forced dead-proxy `refreshUsage` checks run for both Codex and Claude temporary profiles. The smoke now proves both official providers return sanitized `network_unavailable` with `headlessRefresh: true` and `visibleBrowserRequired: false` in the current CachyOS KDE/Wayland session. Authenticated official parsing and manual interruption-page validation remain unchecked.
...
Login preflight outcome progress, 2026-06-04: factored the desktop `Start login` preflight into an explicit Rust outcome containing the returned login status and whether a headed browser launch is allowed. Rust regressions now assert `usage` maps to `already_authenticated` with no headed launch, unavailable/unknown states map to `preflight_unavailable` with no headed launch, and only logged-out/MFA/CAPTCHA states can request the headed login sidecar. Real authenticated profile evidence remains unchecked.
...
Frontend fallback prompt progress, 2026-06-04: frontend fallback `webStatus` handling is now limited to local/preview snapshots, so successful web or merged snapshots cannot show stale login-required notes or `Start login` buttons even if fallback metadata leaks through. Vitest coverage verifies direct web failures still expose headed-login prompts, while successful web/merged snapshots ignore stale fallback login metadata. Real authenticated profile evidence remains unchecked.
...
Login visibility preflight evidence progress, 2026-06-04: `npm run smoke:preflight` now emits sanitized login-visibility automation booleans for headless official refreshes, `Start login` preflight, authenticated no-launch decisions, preflight snapshot updates, official-usage preview prompt hiding, stale login status clearing, web-evidence guards, and frontend regression coverage. The authenticated-web manual evidence template now requires future observations that headed login opens only after user-action states, silent preflight clears stale login prompts, and local-only refreshes do not hide a needed login prompt. Real authenticated profile evidence remains unchecked.
...
- [ ] Web providers fail closed on login, MFA, CAPTCHA, bot checks, unexpected UI, or parse failure.
  ...
  - [x] Real headless Playwright official refresh smoke validates forced Codex and Claude network failures return sanitized `network_unavailable` without opening a visible browser.
...
- [ ] Prove fail-closed handling for logged-out, MFA, CAPTCHA, and unexpected UI states.
  ...
  - [x] `npm run test:official-fail-closed` validates real headless Playwright official refreshes with a forced dead proxy return sanitized `network_unavailable` for both Codex and Claude without opening a visible browser.
...
- [ ] Confirm no sensitive page content is written to normal logs.
  ...
  - [x] `npm run test:auth-profile-helper` validates CLI and environment shared profile-root inputs plus per-service override precedence without exposing the root, derived profile paths, override paths, or log path.
...
- [ ] For manual KDE checks: date/session, OS/session type, artifact/binary used, and observed behavior are recorded.
  ...
  - [x] Add sanitized preflight booleans for login visibility automation coverage and future authenticated login prompt observations.
...
- [ ] For web/session security checks: sanitized inspection notes confirm no secrets or raw authenticated page content are persisted outside browser profiles.
  ...
  - [x] `npm run test:auth-profile-helper` validates the CLI and environment shared browser-profile-root input forms recommended for default app-owned profiles, plus per-service override precedence.
  - [x] `npm run test:auth-profile-helper` validates shared browser-profile-root safety checks reject missing, non-directory, symlink, and default-browser roots before deriving service paths.
