...
Decision: proceed with Playwright headed Chromium sidecar. Local headed sidecar validation proves distinct temporary profile persistence, disabled password/autofill preference preservation across relaunch, no import from seeded fake default Chrome/Chromium profiles, and sanitized stdout/stderr for the launch path. Web provider implementation remains deferred until manual tests prove app-owned authenticated profile persistence, visible login, no saved credentials after login, and parseable official page fields.
