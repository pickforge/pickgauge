...
- [ ] Run `npm --silent run smoke:auth-profile -- --codex-profile <codex-profile> --claude-profile <claude-profile> --require-usage --require-disabled-storage-preferences --require-no-credential-store-files --require-no-autofill-store-files --require-no-default-profile-references` or the equivalent environment-variable form, and record only the sanitized JSON result.
