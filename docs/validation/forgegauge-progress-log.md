...
- Added `npm run test:sidecar-launch`, which launches the generated sidecar against the Codex and Claude official URLs with `headless: false`, distinct temporary isolated profiles under `/tmp`, sanitized stdout-only response checks, and relaunch persistence checks for each service profile.
- Validation: `npm test`, `npm run test:sidecar-launch`, `npm run check`, `npm run build`, `cargo fmt --check`, `cargo check`, `cargo test` (`168 passed`), `cargo clippy -- -D warnings`, and `npm run build:appimage` passed. `npm run test:sidecar-launch` passed for both services, preserved profile sentinel files across relaunch, removed the temporary profile directories afterward, and left no ForgeGauge sidecar processes running.
...
- This proves the local sidecar can start headed Playwright sessions for both official URLs with distinct persistent temporary profiles, but it does not prove authenticated login, app-owned profile persistence, saved-credential absence, parseable authenticated fields, or AppImage runtime behavior outside the source workspace.
