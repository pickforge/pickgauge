...
- This proves the local sidecar can start headed Playwright sessions for both official URLs with distinct persistent temporary profiles and preserved disabled password/autofill preferences, but it does not prove authenticated login, app-owned profile persistence, saved-credential absence after login, parseable authenticated fields, or AppImage runtime behavior outside the source workspace.
