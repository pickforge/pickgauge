...
function writeFixture(root) {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      tag_name: "v9.9.9",
      assets: [{ browser_download_url: "https://example.test/PickGauge_9.9.9_amd64.AppImage" }],
    }),
  );
  writeExecutable(
...
case "$url" in
  *api.github.com*|*release.test*)
    ...
    cat "$PICKGAUGE_TEST_FIXTURE/release.json"
    ;;
  *.AppImage)
    cp "$PICKGAUGE_TEST_FIXTURE/\${url##*/}" "$out"
    ;;
  *)
...
function runInstaller(root, fixture, extraEnv = {}) {
  ...
  const env = {
    ...
    PICKGAUGE_TEST_FIXTURE: fixture,
    PICKGAUGE_RELEASE_API_URL: "https://release.test/latest",
    ...extraEnv,
  };
...
function test(name, fn) {
  const root = makeTempRoot(`pickgauge-installer-${name.replace(/[^a-z0-9]+/gi, "-")}`);
  try {
    fn(root);
    console.log(`ok - ${name}`);
  } finally {
...
test("AppImage install writes a FUSE-aware wrapper, desktop entry, and icon", (root) => {
  ...
  const appImage = join(home, ".local", "bin", "PickGauge.AppImage");
  const command = join(home, ".local", "bin", "pickgauge");
  const launcher = join(home, ".local", "share", "applications", "pickgauge.desktop");
  const icon = join(home, ".local", "share", "icons", "hicolor", "scalable", "apps", "pickgauge.svg");
...
test("AppImage upgrade replaces old symlink command without overwriting the AppImage", (root) => {
  ...
  writeExecutable(join(bin, "PickGauge.AppImage"), "#!/bin/sh\nexit 0\n");
  symlinkSync("PickGauge.AppImage", join(bin, "pickgauge"));

  runInstaller(root, fixture);

  const appImage = readFileSync(join(bin, "PickGauge.AppImage"), "utf8");
  const command = readFileSync(join(bin, "pickgauge"), "utf8");
...
test("release API override does not receive the GitHub token", (root) => {
  ...
  assert.match(output, /PickGauge v9\.9\.9 installed/);
});
...
test("AppImage install refuses to overwrite an unrelated command", (root) => {
  ...
  const bin = join(root, "home", ".local", "bin");
  mkdirSync(bin, { recursive: true });
  writeExecutable(join(bin, "pickgauge"), "#!/bin/sh\nexit 0\n");

  const result = runInstallerFailure(root, fixture);

  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /command path already exists and was not created by PickGauge/);
});
